...
has default_filename => (
    is      => 'ro',
    default => '/usr/local/muscope/lib/conf/muscope.yaml',
);
...
sub _build_filename {
    ...
    return $_[0] || $ENV{'MUSCOPE_CONF'} || $self->default_filename;
}
...
MuScope::Config - Read local configuration information

=head1 SYNOPSIS

  use MuScope::Conf;

  my $config  = MuScope::Conf->new;
